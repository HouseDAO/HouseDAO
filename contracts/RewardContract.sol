...
contract RewardContract is Context, Ownable{
    ...
    uint256 public rewardRefreshTime = 1645480800 + 1 days;//2022-2-23 06:00
    uint256 public dayNumber = 1;
    uint256 public totalReward;
    uint256 public todayReward;
    uint256 public upperLimit = 4000000 ether * 2 / 100;
    ...
    function _refreshRewardDay() private {
        if(block.timestamp < rewardRefreshTime) return;
        rewardRefreshTime += 1 days;
        dayNumber += 1;
        upperLimit = (upperLimit + 4000000 ether) * 2 / 100;
        todayReward = 0;
    }
    ...
    function RewardFunc(address[] memory addrs,uint256[] memory amounts) public onlyOwner {
        ...
        for(uint256 i = 0; i < addrs.length; i++){
            if((todayReward + amounts[i]) > upperLimit) break;
